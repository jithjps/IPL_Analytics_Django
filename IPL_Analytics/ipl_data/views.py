...
def  top4teams_by_wins(request): 
    ...
    top_4_win_count = winner_result.values_list('win_count',flat=True).distinct()[:4]
    win_count4      = list(top_4_win_count)[-1]
    top_4_teams     = winner_result.filter(win_count__gte = win_count4)
    ...
    return JsonResponse ({
                ...
                'top_team'       : top_team,
                'top_win_venues' : top_win_venue_data,
                'top_mom_player' : top_mom_player
            },safe=False)
...
def toss_analytics(request):
    ...
    return JsonResponse ({
            ...
            'team_wise_toss_match_win'  : team_wise_toss_match_win,
            'top_toss_win_team'         : top_toss_win_team
           },safe=False)
...
def  victory_analysis(request):
    ...
    return JsonResponse ({ 
                "top_team_by_run_margin" : top_team_by_run_margin,
                "top_team_by_wkt_margin" : top_team_by_wkt_margin
           },safe=False)
...
def location_analytics(request):
    ...
    to_hosting_venues = match_by_venue.filter(venue_count = top_hosting_count )
    top_venue_names    = to_hosting_venues.values_list('venue',flat=True)
    ...
    return  JsonResponse ({
               "top_venue_names": top_venue_names,
               "venue_status_dict" :venue_status_dict
    },safe=False)  
